...
bool CDLLOutput::EmitBlockDescriptions()
{
	...
	try
	{
		...
		for (CONSTTOKENITR bit = Blocks.begin(); bit != Blocks.end() && m_bStatus; bit++, nCount++)
		{
			...
			if (_ftprintf(m_pFile, _T("\tSetBlockDescription(pBlockDescriptions + %5d, %-20s, %5d, _T(\"%s\") );\n"), nCount, pToken->GetBlockType(), nCount, pToken->GetBlockName()) < 0)
				throw CDFW2GetLastErrorException (Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
		}
		...
		{
			if (_ftprintf(m_pFile, _T("\tconst long PinsCount[%d] = { "), Blocks.size()) < 0)
				throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
			...
			if (_ftprintf(m_pFile, _T(" };\n\n\tif(nBlockIndex >= 0 && nBlockIndex < %d)\n\t\treturn PinsCount[nBlockIndex];\n\n\treturn -1;\n}\n\n"), Blocks.size()) < 0)
				throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
		}
		...
		else
		{
			...
			for (CONSTTOKENITR bit = Blocks.begin(); bit != Blocks.end() && m_bStatus; bit++, nCount++)
			{
				...
				if (_ftprintf(m_pFile, _T("\tcase %d:\n"), nCount) < 0)
					throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
		...
		{
			if (_ftprintf(m_pFile, _T("\tconst long ParametersCount[%d] = { "), Blocks.size()) < 0)
				throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
			...
			for (CONSTTOKENITR bit = Blocks.begin(); bit != Blocks.end() && m_bStatus; bit++)
			{
				...
				long nBlockParametersCount = pToken->ChildrenCount() - pToken->GetPinsCount() + 1; 

				if (_ftprintf(m_pFile, _T("%d"), nBlockParametersCount) < 0)
					throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
			}
...
bool CDLLOutput::EmitVariablesInfo()
{
	...
	try
	{
		...
		for (COMPILERVARSCONSTITR it = SetPoints.begin(); it != SetPoints.end(); it++, nCount++)
		{
			...
			if (_ftprintf(m_pFile, _T("\tSetVarInfo(pVarsInfo + %5d, %5d, _T(\"%s\"), VARUNIT_NOTSET, 0);\n"), 
                                                                                                                    nCount, 
                                                                                                                    it->second->GetIndex(), 
		...
		{
			const CExpressionToken *pToken = it->second->m_pToken;
			if (_ftprintf(m_pFile, _T("\tSetInputVarsInfo(pVarsInfo + %5d, _T(\"%s\"), DEVTYPE_MODEL, _T(\"%s\") );\n"),
				it->second->GetIndex(),
				pToken->GetTextValue(),
                _T("") ) < 0)
		...
		for (COMPILERVARSCONSTITR it = Consts.begin(); it != Consts.end(); it++, nCount++)
		{
			...
			if (_ftprintf(m_pFile, _T("\tSetConstantVarInfo(pVarsInfo + %5d, _T(\"%s\"), %10g, %10g, %10g, %s, %s);\n"),
				nCount,
				pToken->GetTextValue(),
				dMin,
                dMax,
		...
		for (COMPILERVARSCONSTITR it = Outputs.begin(); it != Outputs.end(); it++, nCount++)
		{
			...
			if (_ftprintf(m_pFile, _T("\tSetVarInfo(pVarsInfo + %5d, %5d, _T(\"%s\"), VARUNIT_NOTSET, 1);\n"),
				nCount,
				it->second->GetIndex(),
				pToken->GetTextValue()) < 0)
		...
		for (EQUATIONSCONSTITR it = Equations.EquationsBegin(); it != Equations.EquationsEnd(); it++)
		{
			...
			if (!pToken->IsHostBlock())
			{
				...
				if (_ftprintf(m_pFile, _T("\tSetVarInfo(pVarsInfo + %5d, %5d, _T(\"%s\"), VARUNIT_NOTSET, %d);\n"),
					nCount,
					pEquation->m_nIndex,
	...
	try
	{
		...
		for (EQUATIONSCONSTITR it = Equations.EquationsBegin(); it != Equations.EquationsEnd(); it++)
		{
			...
			if (!pEquation->m_pToken->IsHostBlock())
			{
				...
				if (_ftprintf(m_pFile, _T("\t(SetFunction)(pm, %5d, pArgs->pEquations[%d] - (%s));\n"),
                        pEquation->m_nIndex,
						pEquation->m_nIndex,
						str.c_str() ) < 0)
					throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
		...
		for (EQUATIONSCONSTITR it = Equations.EquationsBegin(); it != Equations.EquationsEnd(); it++)
		{
			...
			if (!pToken->IsHostBlock())
			{
				...
				if (pToken->IsVariable() && !pToken->IsConst())
				{
					...
					if (_ftprintf(m_pFile, _T("\t(SetElement)(pm, %5d, %-40s, -(%s), 0 );\n"),
						pEquation->m_nIndex,
						strCol.c_str(),
						Diff.c_str()) < 0)
						throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
				...
				else
				{
					struct CommentedRow
					{
						...
						CommentedRow(wstring& Value, wstring& Comment) : m_Value(Value), m_Comment(Comment) {};
					};
					...
					for (TOKENLIST::reverse_iterator tkit = pToken->m_Children.rbegin(); tkit != pToken->m_Children.rend(); tkit++)
					{
						...
						if (Equations.GetDerivative(pToken, pChildToken, Diff, strCol))
						{
							...
							else
							{
								...
								if (mit == MatrixRow.end())
									MatrixRow.insert(make_pair(strCol, CommentedRow(Diff, Comment)));
								else
								{
				...
				if (m_bStatus)
				{
                    ...
                    if (Diagonal.empty())
					{
						if (_ftprintf(m_pFile, _T("\t(SetElement)(pm, %5d, %-40d, 1.0, 0);\n"),
							pEquation->m_nIndex,
					...
					{
						if (_ftprintf(m_pFile, _T("\t(SetElement)(pm, %5d, %-40d, 1.0 - (%s), 0);\n"),
							pEquation->m_nIndex,
							pEquation->m_nIndex,
							Diagonal.c_str()) < 0)
							throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
	...
	try
	{
		...
		for (EQUATIONSCONSTITR it = Equations.EquationsBegin(); it != Equations.EquationsEnd(); it++)
		{
			...
			if (pToken->IsHostBlock())
			{
				...
				if (_ftprintf(m_pFile, _T("\t// pArgs->pEquations[%d] = %s;\n"), 
					pEquation->m_nIndex,
					str.c_str()) < 0)
					throw CDFW2GetLastErrorException(Cex(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
