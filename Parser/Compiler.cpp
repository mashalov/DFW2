...
bool CCompilerEquations::GetDerivative(CExpressionToken *pToken, CExpressionToken *pTokenBy, wstring& Diff, wstring& strCol) const
{
	...
	if (bRes)
	{
		if (pTokenBy->m_pEquation)
			...
			pTokenBy->GetEquationOperandIndex(pToken->m_pEquation, strCol);
		bRes = true;
	}
		...
		if (!pToken->IsHostBlock())
		{
			...
			else
			{
				for (TOKENLIST::reverse_iterator tkit = pToken->m_Children.rbegin(); tkit != pToken->m_Children.rend() && bRes; tkit++)
				{
					...
					if (GetDerivative(pToken, pChildToken, Diff, strCol))
					{
