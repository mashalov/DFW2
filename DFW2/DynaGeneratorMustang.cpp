...
bool CDynaGeneratorMustang::BuildEquations(CDynaModel *pDynaModel)
{
	...
	if (bRes)
	{
		...
		// dS / Eqss
		pDynaModel->SetElement(A(V_S), A(V_EQSS), hb0 / Mj * Iq);
		// dS / Edss
		pDynaModel->SetElement(A(V_S), A(V_EDSS), hb0 / Mj * Id);
		// dS / Id
		pDynaModel->SetElement(A(V_S), A(V_ID), hb0 / Mj * (Edss + Iq * (xd2 - xq2)));
		// dS / Iq
		pDynaModel->SetElement(A(V_S), A(V_IQ), hb0 / Mj * (Eqss + Id * (xd2 - xq2)));


		// dEqss / dEqss
		pDynaModel->SetElement(A(V_EQSS), A(V_EQSS), 1.0 + hb0 / Td0ss);
		// dEqss / dEqs
		pDynaModel->SetElement(A(V_EQSS), A(V_EQS), -hb0 * (1.0 / Td0ss - 1.0 / Td01));
		// dEqss / dId
		pDynaModel->SetElement(A(V_EQSS), A(V_ID), -hb0 * ((xd1 - xd2) / Td0ss + (xd - xd1) / Td01));
		// dEqss / dEqe
		if (ExtEqe.Indexed())
			pDynaModel->SetElement(A(V_EQSS), A(ExtEqe.Index()), -hb0 / Td01);
