...
double CDynaModel::FindZeroCrossingToConst(const RightVector* pRightVector, double dConst)
{
	...
	double GuardDiff{ GetZeroCrossingTolerance() * (pRightVector->Rtol * std::abs(dConst) + pRightVector->Atol) };
...
{
	const ptrdiff_t q{ Order() };
	const double h{ H() }, dError1{ pRightVector1->Error }, dError2{ pRightVector2->Error };
...
double CDynaModel::GetZCStepRatio(double a, double b, double c)
{
