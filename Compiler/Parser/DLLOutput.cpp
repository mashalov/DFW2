...
bool CDLLOutput::CreateSourceFile(const _TCHAR *cszFile)
{
	...
	try
	{
		...
		if(!CreateAllDirectories(strDir.c_str()))
			throw CDFW2GetLastErrorException(fmt::format(CAutoCompilerMessages::cszFileOpenError, m_strFilePath.c_str()));
		...
		if (_ftprintf_s(m_pFile, _T("\n#include \"..\\Headers\\DeviceDLL.h\"\n\n") ) < 0)
			throw CDFW2GetLastErrorException(fmt::format(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
...
bool CDLLOutput::EmitDeviceTypeName(const _TCHAR *cszTypeName)
{
	...
	{
		CDFW2GetLastErrorException ex(fmt::format(CAutoCompilerMessages::cszFileWriteError, m_strFilePath.c_str()));
		m_Compiler.m_Logger.Log(ex.Message());
		m_bStatus = false;
	}
	return m_bStatus;
				...
				else
				{
					...
					for (TOKENLIST::reverse_iterator tkit = pToken->m_Children.rbegin(); tkit != pToken->m_Children.rend(); tkit++)
					{
						...
						if (Equations.GetDerivative(pToken, pChildToken, Diff, strCol, bExternalGuard))
						{
							...
							std::wstring DiagIndex = fmt::format(_T("%d"), pEquation->m_nIndex);
							if (DiagIndex == strCol)
							{
								Diagonal = Diff;
								if (Diff == _T("1"))
							...
							{
								std::wstring Comment = fmt::format(_T("{} by {}"), pToken->GetTextValue(), pChildToken->GetTextValue());
