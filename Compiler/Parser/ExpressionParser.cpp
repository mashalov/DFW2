...
// извлекает слежующий токен из выражения
...
// разбор заданного выражения в соответствии с https://en.wikipedia.org/wiki/Shunting-yard_algorithm
// алгоритм расширен для обработки функций с переменным числом аргументов за счет использования стека порядка -
// arity stack. В результате получаем AST в ResultStack
...
bool CExpressionParser::Parse(const _TCHAR* cszExpression)
{
		...
		else if (pToken->IsOperator())						// если оператор
		{
			if (pToken->GetType() == ETT_COMMA)				
			{
				...
				while (!m_ParserStack.empty() && !bMatchParent)
				{
					if (m_ParserStack.top()->GetType() == ETT_LB)
						bMatchParent = true;	// скобку нашли, все ОК
					else
					{
						// строим дерево для оператора до скобки
						eError = BuildOperatorTree();
						// отмечаем, что внутри скобок есть хотя бы один оператор
				...
				if (eError == ETT_UNDEFINED)
				{
					// если ошибок нет, проверяем кратность скобок
					if (!bMatchParent)
						pToken->SetError(ETT_ERROR_NOLEFTPARENTHESIS);
					else
						if (!m_ArityStack.empty())	
							m_ArityStack.top()++;		// если стек порядка есть - добавляем количество аргументов в запятых
						else
			...
			else if (pToken->GetType() == ETT_RB)
			{
				...
				while (!m_ParserStack.empty() && !bMatchParent)
				{
					...
					{
						bMatchParent = true;	// скобку нашли, все ОК
						m_ParserStack.pop();	// удаляем найденную скобку из стека
					}
					else
					{
						// строим дерево до оператора до скобки
						eError = BuildOperatorTree();
						if (eError != ETT_UNDEFINED)
							break;
					}
				...
				if (eError == ETT_UNDEFINED)
				{
					...
					{
						// если в стеке - функция
						if (m_ParserStack.top()->IsFunction())
						{
							// строим для нее дерево
							eError = BuildOperatorTree();
							if (eError == ETT_UNDEFINED)
							{
								if (!m_ArityStack.empty())
									m_ArityStack.pop();		// удаляем для фукнции элемент стека порядка
								else
									pToken->SetError(ETT_ERROR_STACKERROR);
							}
							else
			...
			{
				// если левая скобка - сбрасываем ее в стек как оператор
				m_ParserStack.push(pToken);
			}
			else
			{
				...
				if (!pToken->IsError())
				{
					while (!m_ParserStack.empty())
					{
						...
						{
							// если токен в стеке - правоассоциативный, и его очередность больше текущего токена
							if (pStackToken->Precedence() > pToken->Precedence() && !pToken->IsUnary())
							{
								// строим дерево токена
								eError = BuildOperatorTree();
								if (eError != ETT_UNDEFINED)
								{
									pToken->SetError(eError);
						...
						{
							// если токен левоассоциативный и его очередность больше или равна очередности текущего токена
							if (pStackToken->Precedence() >= pToken->Precedence() && !pToken->IsUnary())
							{
								// строим дерево оператора
								eError = BuildOperatorTree();
								if (eError != ETT_UNDEFINED)
								{
									pToken->SetError(eError);
	...
	if (pToken && !pToken->IsError())
	{
		while (!m_ParserStack.empty())
		{
			// если внезапно нашли левую скобку - ошибка, правой не было
			if (m_ParserStack.top()->GetType() == ETT_LB)
			{
		...
		// очищаем стек парсера
		m_ParserStack = PARSERSTACK();

		// проверяем результат - в стеке результата должен быть один оператор
		if (!pToken->IsError())
	...
	if (pToken && !pToken->IsError())
	{
		// вставляем в стек результатов промежуточный корневой элемент
		// так как выражение обязано иметь корневой элемент
...
// пропуск пробелов в выражении до следующего не пробела
...
eExpressionTokenType CExpressionParser::BuildOperatorTree()
{
	...
	CExpressionToken *pStackToken = m_ParserStack.top();
	// извлекаем количество аргементов функции
	ptrdiff_t nArgsCount = pStackToken->ArgsCount();

	if (pStackToken->IsFunction())
	{
		// если оператор является функцией
		if (!m_ArityStack.empty())
		{
			// если в стеке порядка что-то есть и его глубина
			// не соответствует заданному количеству аргументов
			if (nArgsCount >= 0)
			{
				// выдаем ошибку
				...
				nArgsCount = m_ArityStack.top(); // если количество аргументов не задано - принимаем по глубине стека порядка
		}
	}

	if (eError == ETT_UNDEFINED)
	{
		// если пока ошибок нет проверяем, хватает ли элементов стека для поиска аргументов
		if (m_ResultStack.size() >= static_cast<size_t>(nArgsCount))
		{
			while (nArgsCount--)
			{
				// каждый аргумент добавляем в качестве дочернего к оператору
	...
	if (eError == ETT_UNDEFINED)
	{
		// если ошибок не был, удаляем элемент из стека парсера и переносим в стек результатов
		m_ParserStack.pop();
